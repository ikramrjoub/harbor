...
$id=$_GET['id'];
$whatwedo= getOnce("whatwedo","id=$id");
?>
    ...
    <div class="container py-5">
        <div class="row">
            ...
            <div class="col-md-6 offset-md-3">
                <h3 class="mb-3">Edit Work</h3>
                <div class="card">
                    <div class="card-body p-5">
                        <form action="../user/handlers/db.php" method="post" enctype="multipart/form-data">
                            <div class="form-group">
                              <label>Title</label>
                              <input type="text" name="title" value="<?= $whatwedo['title'] ?> " class="form-control">
                              <input type="hidden" name="id" value="<?= $whatwedo['id'] ?>" class="form-control">
                            </div>
                            <div class="form-group">
                                <label>Description</label>
                                <textarea  name= "description" class="form-control"  rows="3"><?= $whatwedo['description'] ?></textarea>
                            </div>
                            <div class="form-group">
                              <label for="customFile">Choose Image</label>
                              <img src="../user/<?= $whatwedo['img'] ?>" class="w-25"/>
                            ...
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control">
                                  <option value="1"<?php if($whatwedo['status']==1){ ?> selected <?php } ?> >active</option>
                                  <option value="0"<?php if($whatwedo['status']==0){ ?> selected <?php } ?>>not active</option>
                                  
                                </select>
                            </div>
                            <div class="text-center mt-5">
                                <button name="editwhatwedo" type="submit" class="btn btn-primary">Edit</button>
                                <a class="btn btn-dark" href="whatWedo.php">Back</a>
                            </div>
                        </form>
                    </div>
                </div>
