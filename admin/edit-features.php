...
$id=$_GET['id'];
$features= getOnce("features","id=$id");
?>
    ...
    <div class="container py-5">
        <div class="row">
            ...
            <div class="col-md-6 offset-md-3">
                <h3 class="mb-3">Edit Features</h3>
                <div class="card">
                    <div class="card-body p-5">
                        <form action="../user/handlers/db.php" method="post" enctype="multipart/form-data">
                            <div class="form-group">
                              <label>Name</label>
                              <input type="text" name="name" value="<?= $features['name'] ?> " class="form-control">
                              <input type="hidden" name="id" value="<?= $features['id'] ?>" class="form-control">
                            </div>
                            
                            <div class="form-group">
                                <label>Status</label>
                                <select name="status" class="form-control">
                                  <option value="1"<?php if($features['status']==1){ ?> selected <?php } ?> >active</option>
                                  <option value="0"<?php if($features['status']==0){ ?> selected <?php } ?>>not active</option>
                                  
                                </select>
                            </div>
                            <div class="text-center mt-5">
                                <button name="editfeatures" type="submit" class="btn btn-primary">Edit</button>
                                <a class="btn btn-dark" href="features.php">Back</a>
                            </div>
                        </form>
                    </div>
                </div>
